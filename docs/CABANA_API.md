...
| Action                                                                         | Auth                                          | Notes                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createStreamUploadTicket`                                                     | creator (RLS)                                 | Quota-checked BEFORE any Cloudflare call; a row exists from ticket issuance so orphaned assets stay sweepable.                                                                                                                         |
| `attachStreamVideoToPost`                                                      | owner (RLS)                                   | Media-mix re-verified after insert (compensating delete). Re-reads status post-insert to correct a stamp taken before the row existed.                                                                                                 |
| `getStreamVideoStatus`                                                         | owner (RLS)                                   | Bounded polling fallback; terminal rows write nothing and skip Cloudflare entirely.                                                                                                                                                    |
| `getStreamPlayback` / `getStreamPlaybackBatch`                                 | **guest-callable** (`optionalSupabaseAuth`)   | `can_view_post` under the caller's context FIRST, then a service-role read of **READY media only**. A viewer who cannot see the post gets `[]`, never a token. Deny-by-default; per-row failure isolation; tokens cached (TTL 3600 s). |
| `detachStreamVideoFromPost`                                                    | owner (RLS)                                   | 5A.4. Drops `post_media` FIRST, then reclaims the asset. Best-effort remote reclaim — the detach already succeeded.                                                                                                                    |
| `deleteStreamVideo`                                                            | owner (RLS)                                   | **Rejects an ATTACHED video by design** — removing a live post's media must be explicit (use detach).                                                                                                                                  |
| `getStreamOrphanReport` / `sweepStreamOrphans` (`stream-reconcile-actions.ts`) | **admin** (`assertAdmin` + service-role work) | Bounded ≤200 rows; `dryRun` defaults **TRUE**. Never enumerates the Cloudflare account — candidates come only from `stream_videos` rows. Attachment re-checked per video immediately before the destructive call.                      |

`post-actions.ts`: `publishPost` asserts media readiness server-side (`assertPublishableMediaRows`) before the update; `deletePost` reclaims attached Cloudflare assets best-effort after the authoritative delete; `getPostMediaUrls(+Batch)` filter on `storage_bucket` so Stream rows are never signed against the Supabase bucket.

**Token lifetime:** signed playback tokens are issued for **3600 s** (`STREAM_PLAYBACK_TOKEN_TTL_SECONDS`); the client caches for 50 min (10 min of headroom so a token never expires mid-watch). Entitlement is re-checked on every issuance — revoking access prevents new tokens, though an already-issued token remains valid until it expires.
...
- **LIVE in production since July 13, 2026** — registered at `https://cabanagrp.com/api/webhooks/stream`; the secret and customer subdomain are set in Vercel Production + Preview (sensitive type: write-only, never readable back). The webhook is now the primary lifecycle driver; owner polling (`getStreamVideoStatus`) is the bounded fallback. `smoke:prod`'s STREAM-WEBHOOK check (unsigned → 401) PASSes.
