...
- Cloudflare Stream migration `20260536`, ticket/action/webhook/tus/controller foundations, and signed-playback server actions. `CLAUDE.md` says the webhook is registered and live; no successful real-video flow is documented. **[Superseded July 16 2026: the real-video lifecycle was verified end-to-end and the full Stream slice shipped to production — see below.]**
...
Production **also now has** the full Cloudflare Stream slice — composer upload, DB-authoritative publish integrity, and signed playback on feed/profile/discovery/post-detail — via PR #24 (`main` `d2b7137`, July 16 2026), with `20260541` applied to cloud (trigger + narrowed insert grant verified live). The real-video lifecycle passed end-to-end pre-merge (webhook-driven ready, publish enforcement, signed playback, cleanup, independent zero-residue).
...
| Creator directory | Live database rows | Read-only; shipped to production July 15 2026 (PR #25 `15cb8ad`) |
| Creator-page admin writes | Live audited RPCs | Shipped July 15 2026; migrations `20260537`–`20260540` applied to cloud; editor UI live |
...
| Stream webhook/actions | Real provider code, live backend | Real-video lifecycle verified July 16 2026; upload/playback UI shipped (PR #24 `d2b7137`) |
| Stream composer | Production main | Merged July 16 2026 (`d2b7137`); publish gate DB-authoritative via `20260541` |
