...
- **Cloudflare Stream video (live in production July 16, 2026; PR #24 → `main` `d2b7137`; cloud through `20260541`):** video upload (tus) / playback / cleanup with **always-signed playback** (public posts included; tokens fetched client-side via React Query, never SSR'd). The **only** Cloudflare-API-secret-bearing modules are `stream-cloudflare.server.ts` and `stream-webhook.server.ts` (+ `stream-reclaim.server.ts`), imported **dynamically inside server handlers** — never into client bundles. The production webhook (`/api/webhooks/stream`, HMAC-verified, live since July 13, 2026) is the primary lifecycle driver; owner polling is the bounded fallback. Publishing a post with video is enforced **server-side and in the database**: the `publishPost` action asserts readiness from `stream_videos.status`, and the `20260541` trigger + column-scoped `stream_videos` INSERT grant (four ticket columns; no authenticated lifecycle UPDATE) block raw-PostgREST bypass and forged `ready` status. Orphan reconciliation is **admin-triggered** at `/admin/stream` (no cron); cross-reload upload recovery is absent by design; adult-content production uploads remain disabled.
